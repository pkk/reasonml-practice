...
let quantityField = Doc.getElementById("quantity", D.document);

let shirtSizeField = Doc.getElementById("size", D.document);
...
| Some(element) => 
    D.EventTarget.addEventListener("input", calculate, Elem.asEventTarget(element));
| None => ()
};

switch (shirtSizeField) {
| Some(element) => 
    D.EventTarget.addEventListener("input", calculate, Elem.asEventTarget(element));
| None => ()
};